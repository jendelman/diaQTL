...
```
```{r include=FALSE}
load("~/git/diaQTL_vignette_data.rd")
```
...
## Detection threshold
The function `DIC_thresh` can be used to estimate the $\delta$DIC threshold corresponding to a genome-wide false positive rate of 0.05 (`alpha` argument). This function uses a curve fit to simulated datasets with no QTL, which produces results similar to a permutation test (which is also available via the function `scan1_permute`). The genome size for the potato dataset can be estimated from the genetic map.
```{r }
sum(tapply(data@map$cM,data@map$chrom,max))/100 #Morgans 
DIC_thresh(genome.size=12.1,num.parents=3,ploidy=4,alpha=0.05) #threshold
```

## Results
The `scan1_summary` function returns the marker with the lowest $\Delta$DIC score on each chromosome and a plot of the $\Delta$DIC profile. 
```{r}
ans1 <- scan1_summary(scan1_example,thresh=-20,position="bp")
ans1$peaks
ans1$plot
```
